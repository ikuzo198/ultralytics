...
Usage:
    $ yolo task=segment mode=train model=yolov8n-seg.pt imgsz=640 \
    data=ultralytics/datasets/tutorials/data.yaml epochs=10 name=tutorials
"""
    ...
    def _do_train(self, rank=-1, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                # ADD iter-based training
                if rank in {-1, 0}:
                    if ni != 0 and ni % 500 == 0:
                        ...
                        self.metrics, self.iter_fitness = self.validate()
                        iter_path = (
                            self.wdir
                            / f"iter_{ni}_{self.metrics['metrics/mAP50(B)']}.pt"
                        )
                        self.save_model_iter(iter_path)
